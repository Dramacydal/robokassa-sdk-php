...
class Robokassa {
	...
	public function __construct(array $params, HttpClientInterface $httpClient, ?SignatureService $signer = null) {
		...
		if (empty($params['login']))	 throw new RobokassaException('Param login is not defined');
		if (empty($params['password1'])) throw new RobokassaException('Param password1 is not defined');
		if (empty($params['password2'])) throw new RobokassaException('Param password2 is not defined');

		if (!empty($params['is_test'])) {
			if (empty($params['test_password1'])) throw new RobokassaException('Param test_password1 is not defined');
			if (empty($params['test_password2'])) throw new RobokassaException('Param test_password2 is not defined');
			$this->is_test = true;
		}
